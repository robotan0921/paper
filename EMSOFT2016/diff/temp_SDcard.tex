...
%DIF DEL 0212_thesis.tex   Fri Feb 12 18:05:04 2016
%DIF ADD ../thesis.tex     Fri Feb 12 18:35:32 2016
% File: thesis.tex
...
mruby library bytecode modified infrequently is preserved beforehand \DIFdelbegin \DIFdel{in an SD card }\DIFdelend \DIFaddbegin \DIFadd{with the platform in a strage device }\DIFaddend at the time of the first compilation.
...
One of the problems is \DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{low }\DIFaddend software development efficiency\DIFdelbegin \DIFdel{is low because }\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend mruby on TECS only supports \DIFdelbegin \DIFdel{an SD card }\DIFdelend \DIFaddbegin \DIFadd{a strage device }\DIFaddend in the platform for LEGO MINDSTORMS EV3 \cite{par:EV3} to load mruby programs.
\DIFdelbegin \DIFdel{In the current development process, it is annoying that the }\DIFdelend \DIFaddbegin \DIFadd{For example, an }\DIFaddend SD card should be inserted and pulled out repeatedly\DIFdelbegin \DIFdel{and the OS should be restarted.
}\DIFdelend \DIFaddbegin \DIFadd{, or a ROM should be rewritten if mruby programs are modified.
Developers should also restart an OS on a target device.
}\DIFaddend %Moreover, in the current system if developers attempt to run multiple tasks with the same priority, a task with first execution only runs and all other tasks do not run unless developers call the OS's function.
%In addition developers must prepare as many VMs as the number of tasks because one RiteVM supports only one task.
\DIFdelbegin \DIFdel{Although }\DIFdelend \DIFaddbegin \DIFadd{Moreover, although }\DIFaddend mruby on TECS has supported the multi-VM, developers need to call OS's function in order to execute multiple tasks.
\DIFdelbegin \DIFdel{Thus, that is }\DIFdelend \DIFaddbegin \DIFadd{These problem are }\DIFaddend a big burden on developers.
...
To improve the development efficiency, the mruby bytecode loader using Bluetooth enables developers to \DIFdelbegin \DIFdel{insert an SD card }\DIFdelend \DIFaddbegin \DIFadd{write the platform in a strage device }\DIFaddend only once at the beginning and to transfer mruby application codes from a host to a target device.
...
{\bf Contributions}: The proposed framework gives the contribution in the following points.
\begin{enumerate}
\item To improve the software development efficiency: \mbox{}\\
\ Developers do not need to \DIFdelbegin \DIFdel{insert and pull out an SD card repeatedly and }\DIFdelend \DIFaddbegin \DIFadd{rewrite a storage device and }\DIFaddend also to restart an OS.
...
\item To focus on the benefits of component-based developments: \mbox{}\\
...
In the current system, \DIFdelbegin \DIFdel{all binary data including }\DIFdelend \DIFaddbegin \DIFadd{the platform including mruby }\DIFaddend bytecodes are saved in \DIFdelbegin \DIFdel{an SD card}\DIFdelend \DIFaddbegin \DIFadd{a storage device}\DIFaddend .
Developers must \DIFdelbegin \DIFdel{insert and pull out the SD card }\DIFdelend \DIFaddbegin \DIFadd{rewrite the storage device }\DIFaddend every time the application programs are modified.
The OS \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend the target device should be also restarted.
...
Developers should just once carry out the work to \DIFdelbegin \DIFdel{insert the SD card }\DIFdelend \DIFaddbegin \DIFadd{connect the storage device }\DIFaddend and start the OS. 
...
The proposed framework provides the design that only mruby applications are sent and mruby libraries are preserved \DIFdelbegin \DIFdel{in the SD card }\DIFdelend \DIFaddbegin \DIFadd{with the platform in the storage device }\DIFaddend beforehand.
...
The platform including RiteVMs and mruby library is compiled and copied in the \DIFdelbegin \DIFdel{SD card }\DIFdelend \DIFaddbegin \DIFadd{storage device }\DIFaddend at the first.
...
This generated bytecode is prepared beforehand in the \DIFdelbegin \DIFdel{SD card }\DIFdelend \DIFaddbegin \DIFadd{storage device }\DIFaddend such as mruby libraries, and different from a bytecode transferred with Bluetooth.
...
In addition, the design can save the time to \DIFdelbegin \DIFdel{insert an SD card }\DIFdelend \DIFaddbegin \DIFadd{connect a storage device }\DIFaddend to restart an OS.
...
The loader provides developers with the software development efficiency without \DIFdelbegin \DIFdel{disconnecting an SD card }\DIFdelend \DIFaddbegin \DIFadd{rewriting a storage device }\DIFaddend and restarting an OS.
