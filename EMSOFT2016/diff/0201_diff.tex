...
%DIF DEL 0127_thesis.tex   Wed Jan 27 01:03:10 2016
%DIF ADD 0201_thesis.tex   Sun Jan 31 23:03:40 2016
% File: thesis.tex
...
\DIFaddbegin \DIFadd{TECS is written in .cdl file (component description language).
...
Co-routines are scheduled by developers with APIs such as }{\it \DIFadd{resume}} \DIFadd{and }{\it \DIFadd{yield}}\DIFadd{, and thus co-routine does not receive the OS's support.
Besides, }{\it \DIFadd{delay}}\DIFadd{, a service call of $\mu$ITRON, can be used for multitasking, but it leaves the scheduling to developers such as co-routine.
}\DIFaddend In the proposed framework, \DIFdelbegin \DIFdel{multitask processing is user-friendly because developers need the less knowledge than before}\DIFdelend \DIFaddbegin \DIFadd{as an approach for multitask processing, }{\it \DIFadd{rotateReadyQueue}}\DIFadd{, a service call of $\mu$ITRON, is implemented}\DIFaddend .
...
The \DIFdelbegin \DIFdel{rot\_rdq }\DIFdelend \DIFaddbegin {\it \DIFadd{rotateReadyQueue}} \DIFaddend is a service call of $\mu$ITRON.
...
Figure \ref{celltype_cyclic_handler} shows tCyclicMain {\it celltype}, which has a {\it call} port which is connected \DIFaddbegin {\it \DIFaddend tkernel.eiKernel\DIFaddbegin } \DIFaddend to call functions of the kernel, an {\it entry} port and an attribute, priority.
The attribute, priority is used as an arguments of \DIFdelbegin \DIFdel{rot\_rdq}\DIFdelend \DIFaddbegin {\it \DIFadd{rotateReadyQueue}}\DIFaddend .
...
\begin{itemize}
        ...
        \item the comparison of the overheads for each cyclic period of calling \DIFdelbegin \DIFdel{rot\_rdq}\DIFdelend \DIFaddbegin {\it \DIFadd{rotateReadyQueue}}\DIFaddend .
        \item the comparison of the application execution time with singletasking, co-routine\DIFaddbegin \DIFadd{, }\DIFaddend and multitasking.
\end{itemize}
...
\DIFaddend The 100,000 times loop program is used as mruby application for evaluation of execution time.
...
\DIFaddend The comparison of the application execution time with singletasking, co-routine\DIFaddbegin \DIFadd{, }\DIFaddend and multitasking is shown in Figure \ref{fig:comparison_s_c_m}.
In Figure \ref{fig:comparison_s_c_m}, the cyclic time of the cyclic handler for multitasking is \DIFdelbegin \DIFdel{1 }\DIFdelend \DIFaddbegin \DIFadd{one }\DIFaddend msec.
...
\DIFaddbegin \DIFadd{Switching tasks's overhead is also evaluated, that is about three micro seconds on average.
...
The }\DIFdelend \DIFaddbegin \DIFadd{Lua is one of the most popular script languages for embedded systems.
Lua supports co-routines, referred to collaborative multitasking.
A }\DIFaddend co-routine \DIFdelbegin \DIFdel{is supported as same as mruby.
}%DIFDELCMD < 
...
mruby has supported co-routine, but not supported multitasking for RTOSs}\DIFdelend \DIFaddbegin \DIFadd{in Lua is used as an independently executed thread.
