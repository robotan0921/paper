...
%DIF DEL 0221_thesis.tex   Sun Feb 21 19:06:29 2016
%DIF ADD ../thesis.tex     Sun Feb 21 19:59:37 2016
% File: thesis.tex
...
\DIFdelbegin \DIFdel{While multitask's execution time is slower than singletask's, co-routine takes more time than multitask to execute an mruby application.
}\DIFdelend %DIF > While multitask's execution time is slower than singletask's, co-routine takes more time than multitask to execute an mruby application.
This result shows the proposed design is superior to co-routine in terms of execution time.
\DIFdelbegin \DIFdel{The overhead of multitask }\DIFdelend \DIFaddbegin \DIFadd{Moreover, developers can utilize the scheuler practically because the overhead of the RiteVM scheuler }\DIFaddend is about 5 \%\DIFdelbegin \DIFdel{($(multitask-singletask)/multitask$)}\DIFdelend .
...
More than \DIFdelbegin \DIFdel{10 }\DIFdelend \DIFaddbegin \DIFadd{eight }\DIFaddend msec do not be evaluated in this paper because it is thought the larger cyclic time influences applications.
\DIFdelbegin \DIFdel{Each execution time of cyclic period is the same as the others.
That is because the overhead of switching tasks (about 3 $\mu$sec) is small in comparison with the execution time.
This results also shows the overhead becomes smaller as the cyclic time is }\DIFdelend %DIF > Each execution time of cyclic period is the same as the others.
%DIF > That is because the overhead of switching tasks (about 3 $\mu$sec) is small in comparison with the execution time.
\DIFaddbegin \DIFadd{The execution time decreases as the cyclic time become }\DIFaddend larger, because the \DIFdelbegin \DIFdel{overhead depends on the }\DIFdelend number of switching tasks \DIFdelbegin \DIFdel{.
...
The execution time of one msec is only about one \% lager than that of eight msec.
...
%DIF > This results also shows the overhead becomes smaller as the cyclic time is larger, because the overhead depends on the number of switching tasks.
%DIF > The smaller cyclic time is better in multitasking due to concurrent and/or parallel processing.
}\DIFaddend 
...
In terms of .c, (B)'s lines do not increase even if the number of RiteVMs increases, while (A)'s lines increase \DIFdelbegin \DIFdel{in proportion to the number }\DIFdelend \DIFaddbegin \DIFadd{as the number increases}\DIFaddend .
(B)\DIFdelbegin \DIFdel{is not modified, and }\DIFdelend \DIFaddbegin \DIFadd{'s .c file }\DIFaddend can be utilized \DIFaddbegin \DIFadd{without modification, }\DIFaddend regardless of the number of RiteVMs.
Moreover, lines of two .cdl files are equal.
The skillfull component-based development brings this \DIFdelbegin \DIFdel{advantage }\DIFdelend \DIFaddbegin \DIFadd{advantages }\DIFaddend such as the decrease of code lines \DIFaddbegin \DIFadd{and non-modified code}\DIFaddend , which leads high productivity and high maintainability.
...
\begin{table}[t]
    ...
    \begin{tabular}{c||cc|c}
                & (A)       & (B)     & Diff  \\ \hline
        .c \DIFaddbeginFL \DIFaddFL{(Total)      }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{134$+$}\DIFdelendFL 8\DIFdelbeginFL \DIFdelFL{$\times\begin{displaymath}\alpha $  \end{displaymath}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\times$$\alpha$$+$134  }\DIFaddendFL & 130     & \DIFdelbeginFL \DIFdelFL{4$+$}\DIFdelendFL 8\DIFdelbeginFL \DIFdelFL{$\times}\alpha$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\times$$\alpha+$4$$\DIFaddendFL \\
        .\DIFaddbeginFL \DIFaddFL{c (Modified)   }& \DIFaddFL{10$\times\alpha}-$2 }& \DIFaddFL{0   }&  \DIFaddFL{10$\times\alpha$$-$2 $$\\
        \DIFaddFL{.}\DIFaddendFL cdl    & \DIFdelbeginFL \DIFdelFL{25$+$}\DIFdelendFL 18\DIFdelbeginFL \DIFdelFL{$\times\begin{displaymath}\alpha$   \end{displaymath}%DIFDELCMD < & %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\times}\alpha$$+$$$\DIFaddendFL 25   \DIFdelbeginFL \DIFdelFL{$+$}\DIFdelendFL \DIFaddbeginFL & \DIFaddendFL 18$\times}\alpha$\DIFaddbeginFL \DIFaddFL{$+$25 }\DIFaddendFL & 0     \DIFdelbeginFL %DIFDELCMD < \\
%DIFDELCMD <         \multicolumn{3}{l}{  }%%%
\DIFdelendFL \\
