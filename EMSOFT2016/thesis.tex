...
One of the problems is that software development efficiency is low because mruby on TECS only supports an SD card in the platform for LEGO MINDSTORMS EV3 \cite{par:EV3} to load mruby programs.
...
To improve the development efficiency, the mruby bytecode loader using Bluetooth enables developers to insert an SD card only once at the beginning and to transfer mruby application codes from a host to a target device.
...
The proposed framework provides the design that only mruby applications are sent and mruby libraries are preserved in the SD card beforehand.
...
The platform including RiteVMs and mruby library is compiled and copied in the SD card at the first.
...
The loader provides developers with the software development efficiency without disconnecting an SD card and restarting an OS.
