...
This thesis proposes an extended mruby on TECS framework to use for software development of IoT devices such as sensors and actuators.
...
In addition, this thesis proposes two component-based functionalities: a componentized TINET called TINET+TECS and a componentized Two-Level Segregate Fit (TLSF) dynamic memory allocator called TLSF+TECS.
...
Moreover, the TLSF+TECS functionality which obtains statistical information of mruby VM's memory usage and frequency helps developers debug and verify the systems.
...
This thesis proposes an extended framework of mruby on TECS that can be applied to embedded network software development for IoT devices.
...
In addition, this thesis proposes a component-based dynamic memory allocator, TLSF+TECS. 
...
In TLSF+TECS, developer can get statistical information of memory usage and frequency, which is helpful for analyzing the memory operation and finding bugs.
...
Each thread can easily set own heap area.
Moreover, statistical information helps developers debug and verify the system.

{\bf Organization}: The remainder of this thesis is organized as follows.
...
Related work is discussed in Chapter \ref{sec:Related Work} and Chapter \ref{sec:Conclusion} concludes this thesis.
...
{\bf Memory frequency:}
Fig. \ref{fig:EvaluationOfTLSFFrequency} evaluates how much memory size and how often a RiteVM requests.
In this evaluation, the same application as Fig. \ref{fig:EvaluationOfTLSFStatistics} runs for about 30 seconds.
During 30 seconds, the RiteVM requests {\it malloc} quite frequency.
In the case of designating heap areas with fixed-size memory, it is necessary to property configure memory blocks suitable for an application.
For example, 10 blocks of 64 bytes, 20 blocks of 128 bytes, and 3 blocks of 64 kilobytes.
In particular, it is not easy to analyze how many memory blocks are needed with RiteVMs, which consumes memory considerably with a high frequency.
With TLSF+TECS, you only need to prepare a large heap area; therefore, such a workload and useless memory are eliminated.
