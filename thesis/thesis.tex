...
\vspace{14mm}
{\Large Division of Systems Science and Applied Informatics,\\Department of Systems Science,\\Schoolo of Engineering Science, Osaka University}\\
\end{center}
\clearpage
\setcounter{page}{0}
\pagenumbering{roman}
...
In the proposed framework, RiteVM sceduler makes multitasking processing more easy-to-use than that of mruby on TECS.
...
In the current development process, it is annoying that the SD card should be inserted and pulled out repeatedly and the OS should be restarted.
...
Although mruby on TECS has supported the multi-VM, developers need to call OS's function in order to execute multiple tasks.
Thus, that is a big burden on developers.

This paper proposes mruby bytecode loader using Bluetooth and RiteVM scheduler for fairly executing mruby programs.
To improve the development efficiency, mruby bytecode loader using Bluetooth enables developers to insert an SD card only once at the beginning and to transfer mruby application codes from a host to a target device.
RiteVM scheduler enables developers to run multitasking programs more easily than mruby on TECS.
...
The loader supports the countinuous loading, which saves the pairing time for Bluetooth.
...
It is that developers need to be well acquainted with the RTOS if multiple mruby programs execute in multitasking.
In the proposed framework, it is easy for developers to use multitasking because RiteVM sceduler switches tasks cyclically.
Synchronization of multipe tasks is also implemented.
...
RiteVM reads the own bytecode and executes it.
The mruby applications run at the same time because of synchronization processing.
RiteVM scheduler switches tasks as multiple tasks can run in concurrent.
...
In the current system, all binary data including bytecodes are saved in an SD card.
Developers must insert and pull out the SD card every time the application programs are modified.
...
Developers should just once carry out the work to insert the SD card and start the OS. 
...
In the proposed framework, mruby applications are only sent, and mruby libraries are preserved in the SD card beforehand.

The development flow in mruby bytecode loader using Bluetooth is shown in Figure \ref{fig:bluetooth_loader}.
The platform including RiteVM and mruby library is compiled and copied in the SD card at the first.
...
The proposed framework provides mruby bytecode loader using Bluetooth as TECS component.
...
Second, RiteVM reads the bytecode of mruby libraries.
...
Third, RiteVM reads the bytecode of the mruby application transferred with Bluetooth.
The mruby application bytecode is stored in an array of type uint8\_t, which is the same as type char.
...
These advantages lead improvement of the work efficiency.
...
\begin{table}[t]
    ...
    \begin{tabular}{c||c|c|c}
          ...
          Bytecode Size     & 14,044 bytes & 199 bytes  &                    \\ %\hline
          Load Process Time & 305.081 msec & 7.774 msec &                    \\
          Compilation Time  &              &            &                    \\
    \end{tabular}
...
Furthermore, RiteVM scheduler makes multitasking more easily than the current mruby on TECS.
...
RiteVM sceduler has the effectiveness in terms of execution time and ease of use compared with singletasking and co-routine because of the low overhead.

The proposed framework is developed in component-base by TECS.
The facilities such as the RiteVM, the RiteVM scheduler, and the Eventflag are implemented as components.
Developers can easily add or remove the facilities as necessary, and also reuse them.
...
\begin{acknowledgement}
    ...
    I also would like to thank all members of Ushio Laboratory for their kind help and cooperation.
\end{acknowledgement}
