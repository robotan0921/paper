...
%DIF DEL 0203_thesis.tex   Wed Feb  3 18:10:19 2016
%DIF ADD 0208_thesis.tex   Sun Feb  7 22:27:02 2016
% File: thesis.tex
...
CBD (Component-Based Development) is a design technique for constituting reusable components \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Crnkovic:2005:CSE:1062455.1062631}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{par:Crnkovic}}%DIFAUXCMD
}\DIFaddend .
...
\DIFaddbegin \DIFadd{There are TECS \mbox{%DIFAUXCMD
\cite{par:TECS}}%DIFAUXCMD
, AUTOSAR \mbox{%DIFAUXCMD
\cite{url:AUTOSAR}}%DIFAUXCMD
, and SaveCCM \mbox{%DIFAUXCMD
\cite{par:SAVEapproach} }%DIFAUXCMD
as a typical component-based development for embedded systems 
}\DIFaddend 
...
mruby on TECS is a component-based framework for running script program \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{7153813}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{par:mrubyonTECS}}%DIFAUXCMD
}\DIFaddend .
It is integrated two technologies.
One is mruby, which is a script language for embedded systems \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{5959588}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{par:mruby}}%DIFAUXCMD
, \mbox{%DIFAUXCMD
\cite{url:mruby}}%DIFAUXCMD
}\DIFaddend .
The other is TECS (TOPPERS Embedded Component System), which is a component-based framework for embedded systems \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{4208825}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{par:TECS}}%DIFAUXCMD
}\DIFaddend .
...
One of \DIFdelbegin \DIFdel{these }\DIFdelend \DIFaddbegin \DIFadd{the problems }\DIFaddend is that working efficiency is low because mruby on TECS only supports an SD card in the platform for LEGO MINDSTORMS EV3 \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{par:EV3} }%DIFAUXCMD
...
Ruby is a object-oriented script language \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{url:Ruby}}%DIFAUXCMD
}\DIFaddend .
...
As the target RTOS, TOPPERS/HRP2 \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{6913200} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{url:HRP2}}%DIFAUXCMD
, \mbox{%DIFAUXCMD
\cite{par:hr-tecs} }%DIFAUXCMD
}\DIFaddend is used in this paper.
TOPPERS/HRP2 is an RTOS based on \DIFdelbegin \DIFdel{\$micro\$}\DIFdelend \DIFaddbegin \DIFadd{$\mu$}\DIFaddend ITRON with memory protection.
However, mruby on TECS does not depend on the RTOS because TECS supports not only TOPPERS/HRP2 but also the other RTOSs such as OSEK \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{6332346} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{par:OSEK} }%DIFAUXCMD
}\DIFaddend and TOPPERS/ASP \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{url:ASP}}%DIFAUXCMD
}\DIFaddend .

\begin{figure}[t]
    \centering
    \DIFaddbeginFL \includegraphics[width=8cm,clip]{figure/mrubyontecs.pdf}
    \caption{\DIFaddFL{System Model of existing mruby on TECS}}
    \label{fig:mrubyontecs}
\end{figure}
\begin{figure}[t]
    \centering
    \DIFaddendFL \includegraphics[width=8cm,clip]{figure/system_model.pdf}
    \caption{\DIFaddbeginFL \DIFaddFL{Specific }\DIFaddendFL System Model \DIFaddbeginFL \DIFaddFL{of the proposed framework}\DIFaddendFL }
...
\DIFaddbegin \DIFadd{mruby programs are consisted of mruby application and libraries.
...
\DIFaddend The development flow in mruby bytecode loader using Bluetooth is shown in Figure \ref{fig:bluetooth_loader}.
...
A component of mruby bytecode loader using Bluetooth is an extension of the RiteVM component, which is described in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{7153813}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{par:mrubyonTECS}}%DIFAUXCMD
}\DIFaddend .
...
TOPPERS/HRP2 is described in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{6913200}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{url:HRP2}}%DIFAUXCMD
, \mbox{%DIFAUXCMD
...
In this framework, ZMODEM \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{forsberg1988zmodem} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{par:zmodem} }%DIFAUXCMD
}\DIFaddend is used as a binary transfer protocol.
...
{
  /* \DIFdelbeginFL \DIFdelFL{Receive the bytecode via Bluetooth */
  bluetooth_loader();
}%DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdelFL{/* }\DIFdelendFL Declaration variables */
  \DIFdelbeginFL \DIFdelFL{CELLCB	*p_cellcb;
  }\DIFdelendFL mrb_state *mrb;
  mrbc_context *c;
  \DIFdelbeginFL %DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdelFL{if (VALID_IDX(idx)) }%DIFDELCMD < {
%DIFDELCMD <     %%%
\DIFdelFL{p_cellcb = GET_CELLCB(idx);
  }%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdelendFL /* New interpreter instance */
  mrb = mrb_open();
  \DIFdelbeginFL \DIFdelFL{if (mrb == NULL) }%DIFDELCMD < {
%DIFDELCMD <     %%%
\DIFdelendFL /\DIFdelbeginFL \DIFdelFL{* Invalid }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{/ Omit: error check for }\DIFaddendFL mrb_state
  \DIFdelbeginFL \DIFdelFL{*}\DIFdelendFL /\DIFdelbeginFL \DIFdelFL{return;
  }%DIFDELCMD < }
%DIFDELCMD <   %%%
\DIFdelFL{/}\DIFdelendFL * New mruby context */
  c = mrbc_context_new(mrb);
  \DIFdelbeginFL %DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdelFL{if(is_cInit_joined())}%DIFDELCMD < {
%DIFDELCMD <     %%%
\DIFdelendFL /\DIFaddbeginFL \DIFaddFL{/ Omit: initialization of mruby-TECS bridge
  /}\DIFaddendFL * \DIFdelbeginFL \DIFdelFL{Initialization of mruby TECS Bridge }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Receive the bytecode via Bluetooth }\DIFaddendFL */
  \DIFdelbeginFL \DIFdelFL{cInit_initializeBridge( mrb );
  }%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{bluetooth_loader( ATTR_irepApp );
  }\DIFaddendFL /* Load mruby library bytecode and run */
  mrb_load_irep_cxt(mrb, ATTR_i\DIFdelbeginFL \DIFdelFL{rep}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{repLib}\DIFaddendFL , c);
  /* Load mruby application bytecode and run */
  mrb_load_irep_cxt(mrb, \DIFdelbeginFL \DIFdelFL{app_binary_buf}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{ATTR_irepApp}\DIFaddendFL , c);
  \DIFdelbeginFL %DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdelendFL if (mrb->exc) {
    /* Failure to execute */
...
\DIFaddend One of approaches for multitasking is co-routine.
Co-routine is \DIFaddbegin \DIFadd{a cooperative thread, and }\DIFaddend scheduled by developers with the functions such as {\it resume} and {\it yield}\DIFdelbegin \DIFdel{, and thus }\DIFdelend \DIFaddbegin \DIFadd{. 
(Ruby }\DIFaddend co-routine \DIFaddbegin \DIFadd{is defined in class Fiber \mbox{%DIFAUXCMD
\cite{url:co-routine}}%DIFAUXCMD
)
Co-routine is a non-preemptive mulititasking, and }\DIFaddend does not receive the OS's support \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{because developers have to switch tasks manually.
Co-routine can not take advantege of multi core processing.
}

\DIFaddend Besides, as another method, {\it delay}, a service call of \DIFdelbegin \DIFdel{\$micro\$}\DIFdelend \DIFaddbegin \DIFadd{$\mu$}\DIFaddend ITRON, can be used for multitasking\DIFdelbegin \DIFdel{, but it leaves the scheduling to developers such as }\DIFdelend \DIFaddbegin \DIFadd{.
This service call delays the execution of the own task for the time of the argument.
In the approach of }{\it \DIFadd{delay}} \DIFadd{fubction, developers have to schedule as the same as }\DIFaddend co-routine.
\DIFaddbegin 

\DIFaddend In the proposed framework, as an approach for multitask processing, {\it rotateReadyQueue}, a service call of \DIFdelbegin \DIFdel{\$micro\$}\DIFdelend \DIFaddbegin \DIFadd{$\mu$}\DIFaddend ITRON, is implemented.
\DIFaddbegin {\it \DIFadd{rotateReadyQueue}} \DIFadd{rotates the precedence of the task with the priority of the argument.
}\DIFaddend A cyclic handler calls {\it rotateReadyQueue}\DIFdelbegin \DIFdel{that switches tasks}\DIFdelend , which enables developers to develop applications without concern for multitasking.

\subsection{rotateReadyQueue}
The case is assumed that there are two tasks with the same priority, and both \DIFdelbegin \DIFdel{are in a }\DIFdelend \DIFaddbegin \DIFadd{tasks are in an }\DIFaddend infinite loop.
In the current system, when one task is executed first, another task would not be executed.
That is because the task with first execution runs in \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend loop.

\DIFdelbegin \DIFdel{The }%DIFDELCMD < {\it %%%
\DIFdel{rotateReadyQueue}%DIFDELCMD < } %%%
...
Cyclic handlers based on ITRON are described in detail \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
In this case, the cyclic handler is executed when it is generated because the attribute is {\it TA\_STA} \DIFaddbegin \DIFadd{that represents the cyclic handler is in an operational state after the creation}\DIFaddend .
...
If developers do not need the cyclic handler, the .cdl files shown as \DIFdelbegin \DIFdel{Figure \ref{celltype_cyclic_handler} , }\DIFdelend \DIFaddbegin \DIFadd{Figures \ref{celltype_cyclic_handler} and }\DIFaddend \ref{build_cyclic_handler} should be commented out such as {\it //import(\DIFdelbegin \DIFdel{\$<\$**.cdl\$>\$}\DIFdelend \DIFaddbegin \DIFadd{$<$tCyclicHandler.cdl$>$}\DIFaddend );}.
...
\subsection{Synchronization of Multiple \DIFaddbegin \DIFadd{RiteVM }\DIFaddend Tasks}
...
For example, in the case of the four \DIFaddbegin \DIFadd{RiteVM }\DIFaddend tasks, each task sets the flag pattern such as 0x01 (0001), 0x02 (0010), 0x04 (0100)  and 0x08 (1000), and then waits 0x0f (1111) with AND.
\DIFaddbegin 

\DIFaddend In the proposed framework, the set pattern and wait pattern are defined as attributes of the component as shown in Figure \ref{fig:Eventflag}.
This design such as {\it cEventflag\_set(ATTR\_setPattern)} enables the program without \DIFdelbegin \DIFdel{"}\DIFdelend \DIFaddbegin \DIFadd{``}\DIFaddend if" statements \DIFdelbegin \DIFdel{.
The components of eventflag }\DIFdelend \DIFaddbegin \DIFadd{and reuses the identical .c file.
Developers do not need to modify the .c file because the .cdl files in accordance with the number of RiteVMs are prepared. 
In additon, the components of Eventflag }\DIFaddend are built with {\it [optional]} in TECS.
{\it [optional]} means that the codes are run only when the call port is connected.
The \DIFdelbegin \DIFdel{identical file can be used }\DIFdelend \DIFaddbegin \DIFadd{.c file does not be rewritten }\DIFaddend even if developers \DIFdelbegin \DIFdel{would not like to use eventflag. 
In other words, it is not necessary to compile/link the different file.
}\DIFdelend \DIFaddbegin \DIFadd{do not use Eventflag. 
}\DIFaddend These features are also advantages of component-based development.
...
To analyze the advantages \DIFdelbegin \DIFdel{and disadvantages }\DIFdelend of the proposed framework, the evaluations are performed as follows.
\begin{itemize}
    \item the comparison of the size and transfer time between an mruby application including mruby library and not
    \DIFdelbegin \DIFdel{.
        }\DIFdelend \item the comparison of the application execution time with singletasking, co-routine, and multitasking
    \DIFdelbegin \DIFdel{.
        }\DIFdelend \item the comparison of the \DIFdelbegin \DIFdel{overheads }\DIFdelend \DIFaddbegin \DIFadd{overhead }\DIFaddend for each cyclic period of calling {\it rotateReadyQueue}
\DIFdelbegin \DIFdel{.
}\DIFdelend \end{itemize}

\DIFaddbegin \DIFadd{These evaluations are performed in order to indicate that mruby bytecode loader improves the work efficiency, and that the proposed multitask processing effectively executes compared with singletasking or co-routine, and also the overhead of the cyclic period.
}\DIFaddend This paper demonstrates the proposed system on a LEGO MINDSTORMS EV3 (300MHz ARM9-based Sitara AM1808 system-on-a-chip) compiled with gcc 4.9.3 -O2 and mruby version 1.2.0.
\DIFdelbegin \DIFdel{The 100,000 times loop program is used as mruby application for evaluation of execution time.
...
In addition, the design can save the time to insert an SD card \DIFaddbegin \DIFadd{to restart an OS}\DIFaddend .
...
\DIFdelbegin \DIFdel{In }\DIFdelend \DIFaddbegin \DIFadd{The 100,000 times loop program is used as mruby application for evaluation of execution time.
In detail, the singletask program loops 100,000 times, and the multitask and co-routine programs loop 50,000 times in each task.
In }\DIFaddend Figure \ref{fig:comparison_s_c_m}, the cyclic time of the cyclic handler for multitasking is one msec.
...
Switching tasks's overhead is also evaluated, that is about three \DIFdelbegin \DIFdel{\$micro\$}\DIFdelend \DIFaddbegin \DIFadd{$\mu$}\DIFaddend sec on average.
...
Therefore, the \DIFdelbegin \DIFdel{overheads }\DIFdelend \DIFaddbegin \DIFadd{overhead }\DIFaddend of multitasking is 0.5 \% or less, and multitasking can be used without the \DIFdelbegin \DIFdel{overheads}\DIFdelend \DIFaddbegin \DIFadd{overhead}\DIFaddend .
...
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{Each }\DIFaddend execution time of \DIFdelbegin \DIFdel{each cyclic time are much the same }\DIFdelend \DIFaddbegin \DIFadd{cyclic period is the same as the others}\DIFaddend .
That is because the overhead of switching tasks (about 3 \DIFdelbegin \DIFdel{\$micro\$}\DIFdelend \DIFaddbegin \DIFadd{$\mu$}\DIFaddend sec) is small in comparison with the execution time.
...
\begin{figure}[t]
    ...
    \caption{Comparison of the \DIFdelbeginFL \DIFdelFL{overheads }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{overhead }\DIFaddendFL for each cyclic period of calling rotateReadyQueue}
...
\begin{table*}[t]
    ...
    \DIFdelbeginFL %DIFDELCMD < \begin{tabular}{c||c|ccc}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \begin{tabular}{c||c|cccccc}
        ...
        Lua \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\cite{url:Lua}}%DIFAUXCMD
, \mbox{%DIFAUXCMD
\cite{par:Lua}           }%DIFAUXCMD
        ...
        \DIFaddFL{python-on-a-chip \mbox{%DIFAUXCMD
\cite{url:python-on-a-chip} }%DIFAUXCMD
}\DIFaddendFL &            \DIFaddbeginFL &            &            &            &            & \checkmark \DIFaddendFL \\
        Owl system \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\cite{par:owl}                    }%DIFAUXCMD
        ...
        mruby \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\cite{par:mruby}}%DIFAUXCMD
, \mbox{%DIFAUXCMD
\cite{url:mruby}     }%DIFAUXCMD
}\DIFaddendFL &            & \DIFdelbeginFL %DIFDELCMD < & %%%
\DIFdelendFL \checkmark &            \DIFaddbeginFL &            &            & \checkmark \DIFaddendFL \\
        mruby on TECS \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\cite{par:mrubyonTECS}         }%DIFAUXCMD
...
\DIFdelbegin \DIFdel{mruby on TECS \mbox{%DIFAUXCMD
\cite{7153813}}%DIFAUXCMD
,  mruby \mbox{%DIFAUXCMD
\cite{5959588}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{eLua \mbox{%DIFAUXCMD
\cite{url:eLua}}%DIFAUXCMD
,  python-on-a-chip \mbox{%DIFAUXCMD
\cite{url:python-on-a-chip}}%DIFAUXCMD
}\DIFaddend , the Owl system \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{180964}}%DIFAUXCMD
, and Lua \mbox{%DIFAUXCMD
\cite{Ierusalimschy:2007:EL:1238844.1238846}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{par:owl}}%DIFAUXCMD
, mruby \mbox{%DIFAUXCMD
\cite{par:mruby}}%DIFAUXCMD
, \mbox{%DIFAUXCMD
\cite{url:mruby}}%DIFAUXCMD
, and mruby on TECS \mbox{%DIFAUXCMD
\cite{par:mrubyonTECS}}%DIFAUXCMD
}\DIFaddend .

\DIFdelbegin \DIFdel{Lua }\DIFdelend \DIFaddbegin \DIFadd{eLua offers the full implementation of Lua programming language to the embedded systems.
Lua }\DIFaddend is one of the most popular script languages for embedded systems \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{url:Lua}}%DIFAUXCMD
, \mbox{%DIFAUXCMD
\cite{par:Lua}}%DIFAUXCMD
}\DIFaddend .
...
\DIFaddbegin \DIFadd{python-on-a-chip (p14p) is a Python run-time system that uses a reduced Python VM called PyMite.
...
\DIFaddend The Owl system is an embedded Python run-time system.
...
The interpreter of the Owl system is the same as that of python-on-a-chip\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
The proposed multitasking \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{has the effectiveness in terms of execution time }\DIFaddend compared with singletasking and co-routine \DIFdelbegin \DIFdel{.
This experimental result shows the effectiveness of the proposed framework }\DIFdelend because of the low overhead.

The proposed framework is developed in component-base by TECS.
The facilities such as the loader, the cyclic handler, and the \DIFdelbegin \DIFdel{eventflag }\DIFdelend \DIFaddbegin \DIFadd{Eventflag }\DIFaddend are implemented as components.
Developers can easily add or remove the facilities as necessary.
\DIFaddbegin \DIFadd{Component-based development can increase the productivity and decrease the complexity.
}\DIFaddend 

\DIFaddbegin \DIFadd{In the future, the mruby bytecode loader using Bluetooth will be supported to handle multiple bytecodes and run the application in multi-VM.
