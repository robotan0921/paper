...
The proposed framework can enables that that mruby program calls TINET, a TCP/IP protocol stack for use in embedded systems, functions.
...
In TINET, the amount of data copying between each protocol layer is minimized.
...
MDD provides the benefits of re-usable, platform-independent, extensible design, however it needs long development time to build an MDD system.
...
In addition, mruby on TECS supports legacy code such as motor driver.
mruby has the same grammar as Ruby which has advantages to web application development as it uses in Rails framework \cite{url:rubyonrails}.
...
In addition, the RiteVMs, TLSF+TECS, and TLSF+TECS are implemented as components; therefore, developers can add, remove, or reuse their functionalities easily as required.
