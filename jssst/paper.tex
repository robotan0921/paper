...
The proposed framework is an extension of mruby on TECS framework \cite{par:mrubyonTECS}, and utilize two technologies i.e., mruby and TECS.
...
This framework integrates two technologies, mruby and TECS, and enables to develop embedded software using script language without slowdown of the execution time. 
...
\begin{table}[t]
    \centering
\caption{Development procedure of existing and proposed framework}
    \scriptsize
    {\tabcolsep=0.1cm
    \begin{tabular}{c||c}
        ...
        \multicolumn{2}{l}{ (3): Restart the RTOS on target (about 4 sec)} \\
...
Note that the time of restarting the RTOS including initializing device drivers is 4 sec in case of  EV3 platform \cite{par:EV3}
...
Fig.\ref{fig:eval_synchronization} shows the results for two, three, and four RiteVM tasks.
...
As shown in Fig.\ref{fig:eval_synchronization}, the time is within 1 msec, 2 msec, and 3 msec respectively, which indicates successful synchronization of multiple RiteVM tasks.
...
In the proposed framework, we implements a Bluetooth loader for mruby bytecode, a RiteVM scheduler, and synchronization mechanism.
The Bluetooth loader improves software development efficiency because of omitting the process to rewrite a storage/ROM device and restart an RTOS on a target device.
