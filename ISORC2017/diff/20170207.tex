...
To improve the scalability and configurability, this paper proposes TINET+TECS, a component-based TCP/IP protocol stack for embedded systems: TINET componentized with TOPPERS embedded component system (TECS).
...
TINET+TECS utilizes a dynamic connection method of TECS components to satisfy the original TINET specification.
We evaluate the component-based TINET compared with the original TINET.
Experimental results show that the overheads of execution time and memory consumption are low, and that the configurability is improved.
...
TINET supports static generation of CEPs and REPs, which is like sockets.
...
Moreover, TECS adapter supports the legacy codes for existing applications.
...
To demonstrate the low overhead of TINET+TECS, we evaluates the execution time and the memory consumption of TINET+TECS compared with TINET.

Fig. \ref{fig:EvaluationOfExecutionTime} shows the comparison of execution time between TINET and TINET+TECS.
...
The execution time of TINET+TECS is as well as that of TINET; the overhead is about 0000 usec.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=7.0cm,clip]{figure/EvaluationOfExecutionTime.pdf}
    \caption{Execution time of TINET and TINET+TECS}
    \label{fig:EvaluationOfExecutionTime}
\end{figure}

TABLE \ref{tab:EvaluationOfMemoryConsumption} shows the memory consumption of TINET and TINET+TECS.
The memory consumption of TINET+TECS increases about 0000\% compared with TINET.
...
\begin{table}[t]
    ...
    \begin{tabular}{c|r|r}
        ...
        RAM       &   10,000 Bytes  &   10,000 Bytes \\
        ROM       &   10,000 Bytes  &   10,000 Bytes \\
        RAM + ROM &   20,000 Bytes  &   20,000 Bytes \\
        \hline
    \end{tabular}
    \label{tab:EvaluationOfMemoryConsumption}
\end{table}

Next, to demonstrate the improved configurability, we measured the code lines for modification as shown in TABLE \ref{tab:EvaluationOfConfigurabilty}.
...
\begin{table}[t]
    ...
    \begin{tabular}{c|r|r|r}
        ...
        Default          &   10,000 Bytes  &   10,000 Bytes      & 0    \\
        I                &   10,000 Bytes  &   10,000 Bytes      & 2    \\
        I + I\hspace{-.1em}I &   20,000 Bytes  &   20,000 Bytes  & 5    \\
        I + I\hspace{-.1em}I + I\hspace{-.1em}I\hspace{-.1em}I   &   20,000 Bytes  &   20,000 Bytes  & 20 \\
        \hline
    \end{tabular}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=7.0cm,clip]{figure/ComparisonOfDynamicConnection.pdf}
    \caption{Component diagrams of two cases (with/without dynamic connection)}
    \label{fig:ComparisonOfDynamicConnection}
\end{figure}

We evaluated the memory consumption without and with TECS dynamic connection.
...
Fig. \ref{fig:EvaluationOfDynamicConnection} shows the memory consumption of two cases, without/with dynamic connection.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=7.0cm,clip]{figure/EvaluationOfDynamicConnection.pdf}
...
TINET+TECS supports existing applications because of the TECS adapter.
To analyze the overhead of TECS adapter, we measured execution time of API with the adapter such as {\it tcp\_snd\_dat/eAPI\_sendData}.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=7.0cm,clip]{figure/EvaluationOfAdapter.pdf}
...
This paper proposes TINET+TECS, a component-based TCP/IP protocol stack for embedded systems.
...
In addition, this paper presents dynamic connection, a new method of TECS.
The method enables dynamic processing while reducing memory consumption.
...
In the future, the proposed framework will cooperate with mruby on TECS \cite{par:mrubyonTECS} to manage IoT devices.
